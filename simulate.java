...
class State 
{ 
    static int sqt,pqt,s1t,p1t,p2t,clock;
}
class Stats{
    static int wait_time1 = 0;
    static int wait_time2 = 0;
    ...
    public static void show(){
        ...
        System.out.println("Total Busy time for Cashier1 = "+Stats.busy_time_server1+" secs = "+Stats.busy_time_server1/60+" mins");
        System.out.println("Total Busy time for Server2 = "+Stats.busy_time_server3+" secs = "+Stats.busy_time_server3/60+" mins");
        ...
        System.out.println("Cashier 1 Utilization = "+ ((Stats.busy_time_server1*100)/Stats.total_time) +"%");
        System.out.println("Server 2 Utilization = "+ ((Stats.busy_time_server3*100)/Stats.total_time) +"%");
        System.out.println("Server 3 Utilization = "+ ((Stats.busy_time_server4*100)/Stats.total_time) +"%");
        // Total wait time for all orders at each stage
        System.out.println("Total Wait time at Stage 1 for all orders = "+Stats.wait_time1+" secs");
        System.out.println("Total Wait time at Stage 2 for all orders= "+Stats.wait_time2+" secs");
    ...
    public static void t_data_tocsv(){
        
        try {
            BufferedWriter writer = Files.newBufferedWriter(Paths.get(GLOBAL.OUTFILEPATH));
            writer.write("Customer,Arrival,order start,Order end,Prep start,Prep end");
            writer.newLine();
            for (Integer i : t_data.keySet()) {
              writer.write(Integer. toString(i));
              for (int element: t_data.get(i)) {
...
class TIMESDATA 
{
    ...
    public TIMESDATA()
    {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("times.csv"))) {
            ...
            int x1 = (int) (Float.parseFloat(times[0])*60); 
            int x2 = (int) (Float.parseFloat(times[1])*60); 
            int x3 = (int) (Float.parseFloat(times[2])*60); 
            int x4 = (int) (Float.parseFloat(times[3])*60); 

            this.arrival.add(x1);
            this.service1.add(x2);
...
class GLOBAL{
    ...
    static String OUTFILEPATH = "experiment_tdata.csv"; // default data out filepath
}
...
class simulate{
    public static void main (String args[]) throws FileNotFoundException{
        ...
        st.setHeaders("clock", "SQ", "S1","PQ","P1","P2","FEL","SQ","PQ");
        // end table setup
        FEL fel=new FEL(); 
    ...
    { 
        // First display the current row => process the first (emminent) event
        display(fel,sq,pq,st);
        
        // Get emminent event from the fel
        Event emminent=fel.remove(); 
        
        // System.out.println("Showing the emminent event");
        // emminent.show();
        // System.out.println("Next event time ="+emminent.time);
        // System.out.println("Sample size ="+GLOBAL.SAMPLE_SIZE);
        
        // Time elapsed between this event and next emminent event
        int time_elapsed = emminent.time-State.clock;
        Stats.total_time +=time_elapsed;

        // Setting clock to next emminent event time
        ...
        if(emminent.type.equals("A"))   arrival(fel,emminent,sq,pq,times); 
        if(emminent.type.equals("S1"))  endservice(fel,emminent,sq,pq,times); 
        if(emminent.type.equals("P1"))  endpacking(fel,emminent,sq,pq,1,times); 
        if(emminent.type.equals("P2"))   endpacking(fel,emminent,sq,pq,2,times); 
        ...
        // System.out.println("order = "+emminent.order+" sample="+GLOBAL.SAMPLE_SIZE+" and Event="+emminent.type);
        // Continue or end simulation check
        if(emminent.order>=GLOBAL.SAMPLE_SIZE &&  (emminent.type.equals("P1") || emminent.type.equals("P2")))   {
        // if(emminent.order>=GLOBAL.SAMPLE_SIZE+1 )   {
            fel.add(new Event("END-OF-SIM", emminent.time, emminent.order));
            display(fel,sq,pq,st);
        }
        else    {
    ...
    public static void endservice(FEL fel,Event emminent,Queue sq,Queue pq, TIMESDATA times) 
    { 
        
        // Server 1 is free now
        State.s1t--;

        // store service ends time to t_data
        Integer[] temp = Stats.t_data.get(emminent.order);
        temp[2] = State.clock;
        Stats.t_data.put(emminent.order,temp);
        ...
        {   
            ...
            fel.add(new Event("P2",packingEndsAt,emminent.order));
            
            // store stage 2 start time to  t_data
            temp = Stats.t_data.get(emminent.order);
            temp[3] = State.clock;
            Stats.t_data.put(emminent.order,temp);
        ...
        {   
            ...
            // System.out.print("packing ends at"+packingEndsAt);
            fel.add(new Event("P1",packingEndsAt,emminent.order));
    ...
    public static void endpacking(FEL fel,Event emminent,Queue sq,Queue pq, int server, TIMESDATA times) 
    { 
    ...
    public static void arrival(FEL fel,Event emminent,Queue sq,Queue pq, TIMESDATA times) 
    { 
        // some order arrived
        
        // Store arrival info in t_data
        // {arr,stg1_st, stg1_end, stg2_start, stg2_end}
        Integer[] temp = {emminent.time, 0, 0, 0, 0};
        Stats.t_data.put(emminent.order,temp);
        ...
        else if(State.s1t==1)
        {
            ...
            State.sqt++;
            sq.add(emminent.order);

        }
        
        // Shedule new arrival
        if(emminent.order<100){
            int nextarrivalAt = times.arrival.remove();
            fel.add(new Event("A",State.clock+nextarrivalAt,emminent.order+1));
        }
        else{
